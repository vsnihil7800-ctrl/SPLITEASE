...
## Phase 17 — Forgot Password + Email Verification (DONE)

### What was built

**Goal:** Valid email check on signup → verification email sent via Resend → forgot password sends reset link to email → user can change password from the link.

### Backend changes

**`backend/models/User.js`** — 5 new fields added (all `select: false` so they never leak into API responses):
...
**`backend/utils/email.js`** — new Resend email utility:
- `sendVerificationEmail({ to, name, token })` — sends branded HTML email with verify link: `CLIENT_URL/verify-email?token=...` (24h expiry)
- `sendPasswordResetEmail({ to, name, token })` — sends reset link: `CLIENT_URL/reset-password?token=...` (30min expiry)
- Resend client is **lazily initialised** (created on first call, not at module load) so the server doesn't crash on startup if `RESEND_API_KEY` is missing in local dev
- `FROM_ADDRESS` = `SplitEase Stay <onboarding@resend.dev>` (Resend's shared domain — works without a custom domain)

**`backend/controllers/authController.js`** — 4 new endpoints added:

| Endpoint | What it does |
|---|---|
| `GET /api/auth/verify-email?token=...` | Finds user by token + expiry check, sets `isEmailVerified=true`, clears token |
| `POST /api/auth/resend-verification` | Generates new token, resends verify email. Always returns 200 (prevents email enumeration) |
| `POST /api/auth/forgot-password` | Generates reset token (30min), sends email. Always returns 200 |
| `POST /api/auth/reset-password` | Validates token + expiry, hashes new password, clears token |

`registerUser` updated: now generates `emailVerifyToken` + `emailVerifyExpires`, calls `sendVerificationEmail` (wrapped in try/catch — email failure is non-fatal, user still gets their JWT and can request resend).

`normalizeUser()` helper added — all auth endpoints now return consistent `{ id, name, email, upiId, isEmailVerified }` shape.

**`backend/routes/authRoutes.js`** — 4 new routes added (see table above).

**`backend/package.json`** — `resend` added as a dependency.

### Frontend changes (files uploaded mid-session, placed into project)

- **`App.jsx`** — routes added for `/forgot-password`, `/reset-password`, `/verify-email`
- **`api/auth.js`** — 4 new API helpers: `forgotPasswordRequest`, `resetPasswordRequest`, `verifyEmailRequest`, `resendVerificationRequest`
- **`pages/ForgotPassword.jsx`** — email input → POST forgot-password → shows "Check your inbox" confirmation with "try again" link
- **`pages/ResetPassword.jsx`** — reads `?token=` from URL → new password + confirm → POST reset-password → shows success + auto-redirects to login after 2.5s
- **`pages/VerifyEmail.jsx`** — reads `?token=` from URL → GET verify-email → shows loading/success/error states
- **`pages/Login.jsx`** — "Forgot password?" link added below password field
- **`pages/Register.jsx`** — after successful register, shows "Check your inbox" banner (instead of navigating to dashboard) with resend button and "skip for now" escape hatch

### Environment variables (already set on Render)
- `RESEND_API_KEY` = `re_4hFERAva_...` ✅ confirmed in Render dashboard
- `CLIENT_URL` = `https://splitease-taupe.vercel.app` ✅ (used to build links in emails)

### Security notes
- Forgot-password and resend-verification always return HTTP 200 regardless of whether the email exists — prevents attackers from enumerating registered emails
- Tokens are `crypto.randomBytes(32).toString('hex')` — 64 hex chars, 256 bits of entropy
- Verify token: 24h expiry. Reset token: 30min expiry
- Reset token is cleared from DB immediately after use (one-time use)
- `emailVerifyToken` and `passwordResetToken` fields are `select: false` — never returned by any query unless explicitly selected

### Next step
Push this to GitHub → Render auto-deploys backend → test the full flow live on `splitease-taupe.vercel.app`. After confirming emails arrive, next phase is whatever comes next on the roadmap.
