...
const CATEGORIES = ["Rent", "Electricity", "WiFi", "Water", "Maid", "Groceries", "Misc"];

export default function AddBillForm({ members, currentUserId, onCreate, submitting, error }) {
  ...
  // Equal split: which members owe a share (defaults to everyone)
  ...
  const handleSubmit = (e) => {
    ...
    if (splitType === "equal") {
      payload.members = participantIds.map((userId) => ({ userId }));
    } else {
      payload.members = Object.entries(customAmounts)
        .filter(([, v]) => Number(v) > 0)
