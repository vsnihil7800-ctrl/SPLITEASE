        ...
        <div className="rounded-2xl border border-hairline bg-surface p-7 shadow-[0_12px_32px_-16px_rgba(22,36,61,0.15)] text-center">
          ...
          {status === "success" && (
            <>
              ...
              <Link to="/login" className="mt-5 inline-block rounded-lg bg-accent px-5 py-2.5 text-sm font-semibold text-white hover:bg-accent/90">
                Go to sign in
              </Link>
            </>
          )}
          {status === "error" && (
            <>
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-danger-soft text-2xl">❌</div>
              <h1 className="font-display text-xl font-semibold text-ink">Verification failed</h1>
              <p className="mt-2 text-sm text-muted">{message}</p>
              <Link to="/login" className="mt-4 inline-block text-sm font-medium text-accent hover:underline">Back to sign in</Link>
            </>
          )}
        </div>
      </div>
